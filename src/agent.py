...
MAX_TOOL_CALLS = 5
SOUL_PROMPT = "You are a curmudgeonly assistant who is very direct and to the point. If you don't know something, say you don't know."
...
class FavoriteColorTool(BaseTool):
    def __init__(self):
        super().__init__(
            ...
            description="This tool allows you to retrieve the user's favorite color. It takes no input and returns a string with their favorite color.",
            parameters={
                "type": "object",
                "properties": {},
                "required": [],
...
class CalculatorTool(BaseTool):
    def __init__(self):
        super().__init__(
            name="CalculatorTool",
            description="This tool allows you to perform basic arithmetic calculations. It takes a string input with the calculation you want to perform (e.g. '2 + 2') and returns a numeric value with the result of the calculation.",
            parameters={
                "type": "object",
                "properties": {
                    "calculation": {
    ...
    def __call__(self, calculation):
        ...
        try:
            ...
            logger.warning("Error: 'bc' command not found. Please install 'bc' to use the Calculator Tool.")
    ...
    def run_agent_loop(self, api_key):
        ...
        try:
            ...
            while user_input.lower() not in ["exit", "quit"]:
                print()
                response, previous_response_id = self.inference_with_tools(user_input, api_key, previous_response_id)
                print(response)
                print()
                user_input = self.non_empty_input("> ")
    ...
    def inference(self, input, api_key, previous_response_id=None):
        ...
        if response.error:
            ...
            if response.incomplete_details.type == "max_output_tokens":
                logger.warning("The response was cut off because it exceeded the maximum output tokens. Consider increasing the max_output_tokens parameter.")
