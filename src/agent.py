...
def run_agent_loop(api_key):
    ...
    try:
        ...
        while user_input.lower() not in ["exit", "quit"]:
            print()
            response, previous_response_id = inference(user_input, api_key, previous_response_id)
            print(response)
            print()
            user_input = non_empty_input("> ")
...
def inference(input, api_key, previous_response_id=None):
